...
void arx_arm::gripper_control()
{
    ...
    else
    {
        ...
        if(play.is_playing)
        {
            if( play_gripper) //开启
            gripper_spd=45;
            else if ( !play_gripper) //夹取
            gripper_spd=-45;
            gripper_cout=12*(gripper_spd-rv_motor_msg[8].gripper_spd);


            CAN_Handlej.Send_moto_Cmd2(8, 10, 1,   target_pos[6], 0,  0 );usleep(200); 
            // ROS_ERROR(">>>>>>gripper playing>>>>>");
        ...
        }else
        {
            ...
            CAN_Handlej.Send_moto_Cmd2(8, 10, 1,   gripper_pos[0], 0,  0 );usleep(200); 
        }        
